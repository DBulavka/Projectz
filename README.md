...
- Для процессов с `category=game`: каждая `userTask` считается уровнем. Коды уровня задаются на уровне процесса через `PUT /api/processes/{id}/levels/{levelKey}/codes` как сущности с полями `value`, `description`, `difficultyValue`, `difficultyDescription` и одинаковы для всех инстансов. В `POST /api/game/tasks/{taskId}/codes` передаётся `value`; если значение совпадает с любым кодом уровня, попытка считается верной. Все попытки (верные и неверные) сохраняются в истории, а задача закрывается автоматически, когда введены все коды уровня.
